...
def load_image():
    ...
    if file_path:
        # 读取图片并显示
        image = Image.open(file_path).convert("RGBA")  # 转换为RGBA模式，以确保一致的形状
        display_image()
        reset_undo_redo()
...
def replace_color_func(img, target_color, new_color, threshold):
    ...
    # 确保目标颜色是长度为3的数组
    target_color = np.array(target_color[:3])
    new_color = np.array(new_color[:3])
    # 计算颜色距离并创建掩码
    color_distances = np.sqrt(np.sum((img_array[:, :, :3] - target_color)**2, axis=-1))
    mask = color_distances < threshold
    # 创建一个新的颜色数组，与图像的形状匹配
    new_color_array = np.zeros_like(img_array)
    new_color_array[:, :] = new_color
    new_color_array[:, :, 3] = img_array[:, :, 3]  # 保留alpha通道
    # 将匹配的像素替换为新颜色
    img_array[mask] = new_color_array[mask]
    # 更新Image对象
